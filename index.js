...
const connection = `mongodb+srv://palmnestuser:${process.env.mongopassword}@palmnestfarm.muv4e.mongodb.net/PalmnestFarm?retryWrites=true&w=majority&appName=PalmnestFarm`;
app.use(cors());
...
app.post("/blog/blogsupdate", cheackUser, async (req, res) => {
    ...
    let responseCloud1;
    image.split(":")[0] === "data"
        ? (responseCloud1 = await cloudinary.uploader.upload(image))
        : (responseCloud1 = image);
...
app.post("/query/userquery", (req, res) => {
    ...
    query
        ...
        .catch((err) => [res.status(400).send("errro")]);
});
